...
export const useAgentMessaging = (selectedAgent: Agent | null) => {
  ...
  const addEvent = (event: Omit<EventTrace, 'id' | 'timestamp'>) => {
    ...
    setAgentEvents(prev => ({
      ...prev,
      [selectedAgent.id]: [newEvent, ...(prev[selectedAgent.id] || [])].slice(0, 100)
    }));
  };
  ...
  const addMessage = (message: ChatMessage) => {
    ...
    setAgentConversations(prev => ({
      ...prev,
      [selectedAgent.id]: [...(prev[selectedAgent.id] || []), message]
    }));
  };
