 ...
 * - Goal suggestions are mediated by PCG, child decides adoption
 */
...
interface PCGInsightsDashboardProps {
  childId: string;
  childName?: string;
}
...
export default function PCGInsightsDashboard({ childId, childName }: PCGInsightsDashboardProps) {
  ...
  const fetchInsights = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/family/pcg-insights?action=summary&childId=${childId}`);
  ...
  const handleSubmitGoal = async () => {
    ...
    try {
      const res = await fetch('/api/family/pcg-insights', {
        method: 'POST',
          ...
          <>
            ...
            <VStack align="stretch" spacing={2}>
              {insights.goals.suggested.map((goal, idx) => {
                const recommendation = goal.pcgRecommendation;
                ...
                return (
                  ...
                  >
                    <VStack align="start" spacing={0}>
                      ...
                      <Text fontSize="xs" color="gray.500">
                        {recommendation === 'appropriate' ? '✓ PCG approved' :
                         recommendation === 'too_easy' ? '⚠️ May be too easy' :
                         recommendation === 'too_hard' ? '⚠️ May be challenging' :
                         '⏳ Awaiting response'}
                      </Text>
                    </VStack>
            ...
            <VStack spacing={4}>
              <Alert status="info" borderRadius="md" size="sm">
                ...
                <Text fontSize="sm">
                  ...
                  PCG will evaluate if the goal is appropriate.
                </Text>
              </Alert>
