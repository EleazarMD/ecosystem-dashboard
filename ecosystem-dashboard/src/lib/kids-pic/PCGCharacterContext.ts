/**
 * PCG Character Context Provider
 * 
 * Provides AI characters with access to child's PCG data for personalized,
...
import { Pool } from 'pg';
import { getKidsPCGService, type KidsPCGService } from './KidsPCGService';
...
export class PCGCharacterContext {
  private pcgService: KidsPCGService;

  constructor(pool: Pool) {
    this.pcgService = getKidsPCGService(pool);
  }
  ...
  async getCharacterContext(
    ...
    const profile = await this.pcgService.getOrCreateProfile(childId);
    ...
    if (options.includeProgress !== false) {
      const progress = await this.pcgService.getProgress(childId);
    ...
    if (options.includeAchievements !== false) {
      const achievements = await this.pcgService.getAchievements(childId);
    ...
    if (options.includeRecentActivities !== false) {
      const activities = await this.pcgService.getRecentActivities(
        childId,
  ...
  private async getCharacterRelationship(
    ...
    const interactions = await this.pcgService.getCharacterInteractions(childId);
...
// Singleton instance
let pcgCharacterContext: PCGCharacterContext | null = null;

export function getPCGCharacterContext(pool: Pool): PCGCharacterContext {
  if (!pcgCharacterContext) {
    pcgCharacterContext = new PCGCharacterContext(pool);
  }
  return pcgCharacterContext;
}

export default PCGCharacterContext;
