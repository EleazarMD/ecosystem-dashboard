/**
 * Kids Personal Context Graph (PCG) Service
 * 
...
// ============================================================================
// Kids PCG Service Class
// ============================================================================

export class KidsPCGService {
  private pool: Pool;
  ...
  async generateContextSummary(childId: string): Promise<string> {
    ...
    if (context.recentActivities.length > 0) {
      const recentTypes = Array.from(
        new Set(context.recentActivities.slice(0, 5).map(a => a.activityType))
      );
...
// Singleton instance
let pcgServiceInstance: KidsPCGService | null = null;

export function getKidsPCGService(pool: Pool): KidsPCGService {
  if (!pcgServiceInstance) {
    pcgServiceInstance = new KidsPCGService(pool);
  }
  return pcgServiceInstance;
}
