...
export interface ParentalGoal {
  ...
  adoptedAt?: Date;
  pcgRecommendation?: 'appropriate' | 'too_easy' | 'too_hard' | 'not_aligned';
  pcgReasoning?: string;
  createdAt: Date;
}
...
export class PCGSafetyService {
  ...
  /**
   * Check if a parent action is allowed by PCG safety rules
   */
  async checkSafetyRule(
    ...
    const result = await this.pool.query(
      `SELECT * FROM check_pcg_safety_rule($1, $2, $3, $4)`,
  ...
  async logAccess(entry: AccessLogEntry): Promise<void> {
    await this.pool.query(
      `INSERT INTO pcg_parental_access_log 
  ...
  private async updateAccessPatterns(parentId: string, childId: string): Promise<void> {
    ...
    await this.pool.query(
      `INSERT INTO pcg_access_patterns 
  ...
  async generateLearningSnapshot(childId: string, date?: Date): Promise<LearningSnapshot> {
    ...
    const result = await this.pool.query(
      `SELECT * FROM pcg_learning_snapshots WHERE child_id = $1 AND snapshot_date = $2`,
      [childId, dateStr]
    );
  ...
  async getLearningSnapshots(
    ...
    // Check safety rules
    const safetyCheck = await this.checkSafetyRule(parentId, childId, 'pcg_learning_snapshots', 'view');
    ...
    await this.logAccess({
      ...
      resourceType: 'pcg_learning_snapshots',
      accessSource
    });

    const result = await this.pool.query(
      `SELECT * FROM pcg_learning_snapshots 
       WHERE child_id = $1 
  ...
  async generateWellnessIndicators(childId: string, date?: Date): Promise<WellnessIndicator> {
    ...
    const result = await this.pool.query(
      `INSERT INTO pcg_wellness_indicators 
  ...
  async getWellnessIndicators(
    ...
    // Check safety rules
    const safetyCheck = await this.checkSafetyRule(parentId, childId, 'pcg_wellness_indicators', 'view');
    if (!safetyCheck.allowed) {
      throw new Error(`Access denied: ${safetyCheck.reason}`);
    }
    ...
    await this.logAccess({
      ...
      resourceType: 'pcg_wellness_indicators',
      accessSource
    });

    const result = await this.pool.query(
      `SELECT * FROM pcg_wellness_indicators 
       WHERE child_id = $1 
   ...
   * PCG evaluates appropriateness, child decides adoption
   */
  async suggestGoal(
    parentId: string,
    childId: string,
    ...
    // Check safety rules
    const safetyCheck = await this.checkSafetyRule(parentId, childId, 'pcg_parental_goals', 'suggest');
    if (!safetyCheck.allowed) {
      throw new Error(`Access denied: ${safetyCheck.reason}`);
    }
    ...
    const pendingCount = await this.pool.query(
      `SELECT COUNT(*) FROM pcg_parental_goals 
    ...
    await this.logAccess({
      ...
      resourceType: 'pcg_parental_goals',
      accessSource: 'dashboard'
    });

    // Insert goal suggestion
    const result = await this.pool.query(
      `INSERT INTO pcg_parental_goals 
       (child_id, parent_id, title, description, category, target_value, target_unit, target_date, pcg_recommendation, pcg_reasoning)
  ...
  async getParentalGoals(
    ...
    // Check safety rules
    const safetyCheck = await this.checkSafetyRule(parentId, childId, 'pcg_parental_goals', 'view');
    if (!safetyCheck.allowed) {
      throw new Error(`Access denied: ${safetyCheck.reason}`);
    }

    let query = `SELECT * FROM pcg_parental_goals WHERE child_id = $1`;
  ...
  private async getGoalsSummary(childId: string) {
    const countsResult = await this.pool.query(
      `SELECT 
       ...
       FROM pcg_parental_goals
       WHERE child_id = $1`,
      [childId]
    );

    const suggestedResult = await this.pool.query(
      `SELECT * FROM pcg_parental_goals 
  ...
  private mapParentalGoal(row: any): ParentalGoal {
    const recommendation = row.pcg_recommendation;
    const reasoning = row.pcg_reasoning;
    ...
    return {
      ...
      pcgRecommendation: recommendation,
      pcgReasoning: reasoning,
...
let pcgSafetyServiceInstance: PCGSafetyService | null = null;

export function getPCGSafetyService(pool: Pool): PCGSafetyService {
  if (!pcgSafetyServiceInstance) {
    pcgSafetyServiceInstance = new PCGSafetyService(pool);
  }
  return pcgSafetyServiceInstance;
}
