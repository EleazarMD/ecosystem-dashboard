...
export default function KnowledgeGraphRedesign() {
  ...
  const loadMetrics = async () => {
    try {
      // Use Vector Search Agent for RAG metrics
      const response = await fetch('http://localhost:41242/a2a/message', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          type: 'get_metrics',
  ...
  const loadSystemStatus = async () => {
    ...
    try {
      // Check Vector Search Agent (RAG)
      const ragCheck = await fetch('http://localhost:41242/health');
      status.ragAgent = ragCheck.ok;
    } catch {}

    try {
      // Check IDE Memory Backend (correct port)
      const mcpCheck = await fetch('http://localhost:9579/health');
    ...
    try {
      ...
      const kgCheck = await fetch('http://localhost:8765/health');
      status.postgresql = kgCheck.ok;
    } catch {}
  ...
  const handleStartSystem = async () => {
    toast({
      title: 'Starting Knowledge Graph System',
      description: 'Run: cd /Users/eleazar/Projects/AIHomelab/core/knowledge-graph && ./scripts/start-kg-complete.sh',
      status: 'info',
      duration: 5000,
      isClosable: true,
    });
    ...
    try {
      // Attempt to start via API (if available)
      const response = await fetch('http://localhost:8765/api/system/start', {
        method: 'POST'
      });
      ...
      if (response.ok) {
        toast({
          ...
          description: 'Knowledge Graph services are starting...',
          status: 'success',
          duration: 3000,
        });
      ...
      toast({
        ...
        description: 'Please run the start script in your terminal',
        status: 'warning',
        duration: 5000,
      });
    }
  ...
  const handleStopSystem = async () => {
    toast({
      title: 'Stopping Knowledge Graph System',
      description: 'Run: cd /Users/eleazar/Projects/AIHomelab/core/knowledge-graph && ./scripts/stop-kg-complete.sh',
      status: 'info',
      duration: 5000,
      isClosable: true,
    });
    ...
    try {
      // Attempt to stop via API (if available)
      const response = await fetch('http://localhost:8765/api/system/stop', {
        method: 'POST'
      });
      ...
      if (response.ok) {
        toast({
          ...
          description: 'Knowledge Graph services are shutting down...',
          status: 'success',
          duration: 3000,
        });
        setTimeout(loadSystemStatus, 3000);
      ...
      // Show manual command
      toast({
        title: 'Manual Stop Required',
        description: 'Please run the stop script in your terminal',
        status: 'warning',
        duration: 5000,
      });
    }
