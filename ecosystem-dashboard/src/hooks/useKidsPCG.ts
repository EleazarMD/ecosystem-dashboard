/**
 * Kids PCG (Personal Context Graph) React Hook
 * 
 * Provides easy access to the PCG system from React components.
...
export function useKidsPCG() {
  ...
  const fetchProfile = useCallback(async () => {
    try {
      const res = await fetch('/api/child/pcg?action=profile');
      if (res.ok) {
  ...
  const fetchContext = useCallback(async () => {
    try {
      const res = await fetch('/api/child/pcg?action=context');
      if (res.ok) {
        const data = await res.json();
  ...
  const fetchContextSummary = useCallback(async (): Promise<string> => {
    try {
      const res = await fetch('/api/child/pcg?action=context-summary');
      if (res.ok) {
        const data = await res.json();
        return data.summary;
      }
  ...
  const fetchProgress = useCallback(async (category?: string) => {
    try {
      const url = category 
        ? `/api/child/pcg?action=progress&category=${category}`
        : '/api/child/pcg?action=progress';
  ...
  const fetchAchievements = useCallback(async () => {
    try {
      const res = await fetch('/api/child/pcg?action=achievements');
      if (res.ok) {
        const data = await res.json();
  ...
  const addKnowledge = useCallback(async (entry: {
    ...
    try {
      const res = await fetch('/api/child/pcg?action=knowledge', {
        method: 'POST',
  ...
  const searchKnowledge = useCallback(async (query: string): Promise<KnowledgeEntry[]> => {
    try {
      const res = await fetch(`/api/child/pcg?action=search-knowledge&q=${encodeURIComponent(query)}`);
      if (res.ok) {
        const data = await res.json();
        return data.results;
      }
  ...
  const updateProgress = useCallback(async (
    ...
    try {
      const res = await fetch('/api/child/pcg?action=progress', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const logActivity = useCallback(async (activity: {
    ...
    try {
      const res = await fetch('/api/child/pcg?action=activity', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const recordCharacterInteraction = useCallback(async (
    ...
    try {
      const res = await fetch('/api/child/pcg?action=character-interaction', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
